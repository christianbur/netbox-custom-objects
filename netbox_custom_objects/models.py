...
from netbox_custom_objects.choices import (
    CustomObjectFieldTypeChoices,
...
class CustomObjectType(NetBoxModel):
    ...
    link_table = models.BooleanField(
        default=False,
        verbose_name=_("Link table (show connected objects)"),
        help_text=_(
    ...
    views = models.CharField(
        ...
        help_text=_(
            ...
            "views registry (built-in plus enabled bundles). A newly "
    ...
    def get_view_keys(self):
        """Selected COT-view registry keys for this type (parsed from ``views``)."""
        return [part.strip() for part in (self.views or "").split(",") if part.strip()]

    def get_proxy_field(self):
        """Return this type's first ``object_proxy`` field, or ``None``.

        A Custom Object Type is treated as a *proxy* type as soon as it owns an
        ``object_proxy`` field: instead of holding its own instances, its list
        page renders a live, read-only projection of another NetBox model.
        """
        return (
            self.fields(manager="objects")
            .filter(type=TYPE_OBJECT_PROXY)
            .order_by("weight", "name")
            .first()
        )

    def is_proxy(self):
        """True if this type has at least one ``object_proxy`` field."""
        return self.get_proxy_field() is not None
    ...
    def _fetch_and_generate_field_attrs(
        ...
        for field in fields:
            # Proxy fields are virtual: they never produce a DB column or model
            # attribute (the type holds no instances of its own), so skip them
            # entirely during model generation.
            if field.type == TYPE_OBJECT_PROXY:
                continue
...
class CustomObjectTypeField(CloningMixin, ExportTemplatesMixin, ChangeLoggedModel):
    ...
    def clean(self):
        ...
        elif self.type == TYPE_OBJECT_PROXY:
            # Proxy fields target a single NetBox model and are never polymorphic.
            if self.is_polymorphic:
                raise ValidationError(
                    {"is_polymorphic": _("Object Proxy fields cannot be polymorphic.")}
                )
            if not self.related_object_type:
                raise ValidationError(
                    {
                        "related_object_type": _(
                            "Object Proxy fields must define an object type."
                        )
                    }
                )
        ...
        # Object Proxy is exclusive ("one and only"): a proxy type renders a
        # read-only projection of another model and holds no instances, so it
        # may have exactly one field — the proxy itself.  Enforce defensively
        # here for non-form saves (API/programmatic); the form sets
        # ``_proxy_exclusivity_checked`` so the GUI flow doesn't report it twice.
        if self.custom_object_type_id and not getattr(
            self, "_proxy_exclusivity_checked", False
        ):
            siblings = self.custom_object_type.fields(manager="objects").all()
            if self.pk:
                siblings = siblings.exclude(pk=self.pk)
            if self.type == TYPE_OBJECT_PROXY:
                if siblings.exists():
                    raise ValidationError(
                        {
                            "type": _(
                                "An Object Proxy field must be the only field on its "
                                "custom object type, but this type already has other "
                                "fields."
                            )
                        }
                    )
                ...
                raise ValidationError(
                    {
                        "type": _(
                            "This custom object type has an Object Proxy field and "
                            "cannot have any additional fields."
                        )
                    }
                )
    ...
    def save(self, *args, **kwargs):
        ...
        # Proxy fields are virtual: they have no backing DB column and never
        # change the generated model's schema.  Persist the row and bump the
        # COT cache_timestamp (so is_proxy()/the list view observe the change),
        # but skip all schema-editor DDL.
        if self.type == TYPE_OBJECT_PROXY:
            with transaction.atomic(using=schema_conn.alias):
                self.custom_object_type.clear_model_cache(self.custom_object_type.id)
    ...
    def delete(self, *args, **kwargs):
        # Proxy fields have no DB column/table to drop — just bump the COT cache
        # and remove the row.
        if self.type == TYPE_OBJECT_PROXY:
            self.custom_object_type.clear_model_cache(self.custom_object_type.id)
            self.custom_object_type.snapshot()
...
class Bundle(models.Model):
    """Activation record for a COT bundle discovered under the local path.

    A bundle is a Python package on the configured local path that bundles
    COT schema YAML definitions and COT-view classes.  Discovery (reading the
    filesystem) is independent of this record; this row only stores whether the
    bundle is *enabled*.  Because views/URLs/menus are registered at worker
    startup, enabling or disabling a bundle only takes full effect after a
    worker restart.
    """
    ...
    restart_required = models.BooleanField(
        default=False,
        help_text=(
            "Set when enabled/disabled until NetBox workers restart and apply "
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            "plugins:netbox_custom_objects:bundle", args=[self.pk]
        )
