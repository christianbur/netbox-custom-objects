...
from netbox_custom_objects.choices import ObjectFieldOnDeleteChoices, TYPE_OBJECT_PROXY
...
class ObjectProxyFieldType(FieldType):
    """Virtual field type for the plugin-only ``object_proxy`` field.

    A proxy field holds no data and produces no DB column, model attribute,
    form/table/serializer field or filter — it only declares the target NetBox
    model (``related_object_type``) and an optional ``related_object_filter``
    that a COT view reads to render a live, read-only projection.  Every
    representational hook therefore raises ``NotImplementedError`` (the value
    every caller already handles by skipping the field gracefully); model
    generation and field save/delete skip proxy fields outright.
    """

    def get_model_field(self, field, **kwargs):
        raise NotImplementedError("object_proxy fields have no model field")

    def get_form_field(self, field, **kwargs):
        raise NotImplementedError("object_proxy fields have no form field")

    def get_filterform_field(self, field, **kwargs):
        raise NotImplementedError("object_proxy fields have no filter field")

    def get_table_column_field(self, field, **kwargs):
        raise NotImplementedError("object_proxy fields have no table column")

    def get_serializer_field(self, field, **kwargs):
        raise NotImplementedError("object_proxy fields have no serializer field")
...
FIELD_TYPE_CLASS = {
    ...
    TYPE_OBJECT_PROXY: ObjectProxyFieldType,
}
