...
class CustomObjectsPluginConfig(PluginConfig):
    ...
    default_settings = {
        ...
        # Filesystem root scanned for COT bundles (Python packages bundling
        # COT schema YAML + COTView classes).  Must be on the PYTHONPATH.
    ...
    def ready(self):
        ...
        # Register one top-level navigation menu per distinct CustomObjectType
        # menu_name.  Must run after super().ready() has registered the stock
        # Custom Objects menu (so the new menus are appended alongside it) and
        # before the navigation snapshot is read by the first request.
        try:
            from netbox_custom_objects.navigation import register_menu_name_menus
            register_menu_name_menus()
        except Exception:
            logger.exception("could not register per-menu-name navigation menus")

        # Import enabled COT bundles (registers their COTView classes) and
        # apply their bundled COT schema documents.  Must run after the COT model
        # passes above so newly created types resolve cleanly; view tabs/menus are
        # request-time/template-driven, so no URLconf freeze ordering applies here.
        try:
            from netbox_custom_objects.cot_views.local_bundles import load_enabled_bundles
            load_enabled_bundles()
        except Exception:
            logger.exception("could not load COT bundles")
